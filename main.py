...
#%%
x_data, label = load_data(args.data, batch_size=args.batch_size)
...
def euclid_distance_func(input_tensor):
    distances = torch.stack([torch.linalg.norm(input_tensor[n]-input_tensor[m]) for n, m in itertools.combinations(range(args.batch_size), 2)], dim=0).cuda()
    return distances

def get_kld(m1, v1, m2, v2):
    output = torch.distributions.kl_divergence(torch.distributions.MultivariateNormal(m1, torch.diag_embed(v1)), torch.distributions.MultivariateNormal(m2, torch.diag_embed(v2)))
    return output

def jsd_distance_func(mu, logvar):
    var = logvar.exp()
    distances = torch.stack([0.5*(get_kld(mu[n], var[n], mu[m], var[m])+get_kld(mu[m], var[m], mu[n], var[n])) for n, m in itertools.combinations(range(args.batch_size), 2)], dim=0).cuda()
    return distances

def rank_func(distance):
    return torchsort.soft_rank(distance.view(1, -1)*100000)
...
elif args.model in ['vae', 'raw_input_and_external_dm_prob']:
...
if args.model =='only_raw_input':
    ...
    def custom_loss(original, latent, output):
        ...
        x_rank, z_rank = rank_func(euclid_distance_func(original)), rank_func(euclid_distance_func(latent))
    ...
    external_dm = distance_matrix(x_data, x_data, p=2)#external distance matrix
    ...
    def custom_loss(original, latent, output, d_m):
        ...
        x_rank, z_rank = rank_func(get_upper_triangular(d_m)), rank_func(euclid_distance_func(latent))
        x2z = -1 * (cos(x_rank - x_rank.mean(), z_rank - z_rank.mean()) - 1)
    ...
    external_dm = distance_matrix(x_data, x_data, p=2)
    x_data = np.column_stack([x_data, np.array(list(range(len(x_data))))])
    x_dim = x_data.shape[1] - 1
    def custom_loss(original, latent, output, mu, logvar, d_m):
        rec_error = F.binary_cross_entropy_with_logits(output, original.view(-1, x_data.shape[1]-1), reduction='sum')
        x_rank, z_rank = rank_func(get_upper_triangular(d_m)), rank_func(jsd_distance_func(mu, logvar))
        x2z = -1 * (cos(x_rank - x_rank.mean(), z_rank - z_rank.mean()) - 1)
        reg_error = args._lambda * x2z
        return rec_error, reg_error
    def get_loss(data, model):
...
print(model)
make_directories()
save_globals_and_functions(vars(args), [euclid_distance_func, rank_func, custom_loss, get_loss])
